...
def keep_conversation_alive(emp_id, request):
    employee_topics = EmployeeTopicSerializer(
        EmployeeTopic.objects.filter(employee=emp_id), many=True).data
    ...
    if len(topics) > 0:
        ...
        answer = compute_answer(
            request, f'Ask me a question about {subject["answer"]}', emp_id)
